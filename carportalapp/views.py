...
def userlogout(request):
	...
	return render(request,'login.html',{"success_message":"Logged-out successfully"})
...
def addProductbycustomermethod(request):
	...
	myfileVideo = request.FILES.get('myfileVideo')
	...
	if (myfile.size) <= 5242880:
		...
		if file_ext in file_types:
			...
			#replace /static
			final_url = uploaded_file_url.replace("/static", "")

	filenameVideo = fs.save(myfileVideo.name, myfileVideo)
	uploaded_file_url_video = fs.url(filenameVideo)
	#replace /static
	final_urlVideo = uploaded_file_url_video.replace("/static", "")

	insertinaddProductmethodUser(product_name,product_price,product_description,product_quantity,product_location,product_model,manufacturing_date,product_condition,product_color,product_url,user_id,final_url,final_urlVideo)
